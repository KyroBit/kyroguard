...
const cache = new Map();
export function clearPolicyCache(policyGroupId) {
    if (policyGroupId)
        cache.delete(policyGroupId);
    else
        cache.clear();
}
async function getGroupPolicies(db, policyGroupId) {
    if (cache.has(policyGroupId))
        return cache.get(policyGroupId);
    const rows = await db
...
export function requirePolicy(policyName, options, rbacOptions) {
    return async (req, reply) => {
        ...
        if (group.is_active === 'false')
            return reply.status(403).send({ message: 'Policy group is inactive' });
        if (group.is_super === 'true')
            return; // owner — allow all
        ...
        if (options?.resource) {
            ...
            if (store && resource) {
                ...
                const resourceDef = rbacOptions.resources.find(r => r.context?.[store.context]?.[policyName] !== undefined);
                ...
                if (scopeNames.length) {
                    ...
                    const [row] = await rbacOptions.db
                        ...
                        .from(policyGroupPolicies)
                        .innerJoin(policies, eq(policyGroupPolicies.policy_id, policies.id))
                        .where(and(eq(policyGroupPolicies.policy_group_id, policyGroupId), eq(policies.name, policyName)));
...
async function checkScopeAgainstResource(condition, resource, db) {
