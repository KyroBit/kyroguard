...
import type { DomainConfig } from '../../core/config.js'
import type { ResolvedGuardConfig } from '../load-config.js'
import type { Policy, ResourceDefinition } from '../../core/policy.js'

/** Typegen only — never calls the adapter, never opens a database. */
export async function run(config: ResolvedGuardConfig, baseDir: string): Promise<void> {
