...
export interface GuardTypes {}

/** The augmented Domain union, or string before augmentation. */
export type DomainName = GuardTypes extends { Domain: infer P extends string } ? P : string

/** The augmented PolicyName union, or string before augmentation. */
export type AnyPolicyName = GuardTypes extends { PolicyName: infer P extends string } ? P : string

/** The policy union for one domain, or string before augmentation. */
export type DomainPolicyName<P extends string> = GuardTypes extends { DomainPolicies: infer M }
  ? P extends keyof M
    ? M[P] & string
    : string
  : string
