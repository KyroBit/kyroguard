...
describe('policyCacheKey collision-proofing', () => {
  ...
  test("':' inside a component cannot fake a component boundary", () => {
    ...
    const domainPos = policyCacheKey('u1', 'admin', '')
    const tenantPos = policyCacheKey('u1', '', 'admin')
    expect(smuggled.id).not.toBe(domainPos.id)
    expect(smuggled.id).not.toBe(tenantPos.id)
    expect(domainPos.id).not.toBe(tenantPos.id)
  })
  ...
  test('key carries the raw (unencoded) components', () => {
    ...
    expect(k.domain).toBe('p 2')
    expect(k.tenantId).toBe('c/3')
  })
...
describe('subjectKeyPrefix', () => {
  const domains = ['', 'admin', 'a:b', '用户']
  const tenants = ['', 't1', 'c:d', '🙂']

  test("is a prefix of every one of that subject's key ids", () => {
    const prefix = subjectKeyPrefix('u1')
    for (const domain of domains) {
      for (const tenantId of tenants) {
        expect(policyCacheKey('u1', domain, tenantId).id.startsWith(prefix)).toBe(true)
      }
    }
  })
  ...
  test("is NOT a prefix of any other subject's key ids (v0 startsWith over-clear regression)", () => {
    ...
    for (const other of ['u1x', 'u', 'u1:', 'u12']) {
      for (const domain of domains) {
        for (const tenantId of tenants) {
          expect(policyCacheKey(other, domain, tenantId).id.startsWith(u1Prefix)).toBe(false)
        }
      }
    }
  })
  ...
  test("shorter subject's prefix does not match a longer subject's keys", () => {
    ...
    for (const domain of domains) {
      for (const tenantId of tenants) {
        expect(policyCacheKey('u1', domain, tenantId).id.startsWith(uPrefix)).toBe(false)
      }
    }
  })
